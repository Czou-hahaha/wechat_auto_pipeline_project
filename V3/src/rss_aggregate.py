...
async def aggregate_rss_from_data_sources(settings: Settings) -> list[SearchHit]:
    ...
    parts_rss = await asyncio.gather(*[bounded(r) for r in feed_rows]) if feed_rows else []
    parts_list = await asyncio.gather(*[bounded_list(r) for r in list_rows]) if list_rows else []
    merged: list[SearchHit] = []
    # 栏目页优先入池，避免与大体量 RSS 竞争 ``rss_total_max`` 时被截断
    for chunk in parts_list:
        merged.extend(chunk)
    for chunk in parts_rss:
        merged.extend(chunk)
    merged = _dedupe_hits(merged, max_results=total_cap)
    logger.info(
        "rss aggregate: rss_feeds=%d html_list=%d rows=%d filter=%s",
        len(feed_rows),
        len(list_rows),
        len(merged),
        relevance,

...
flowchart TB
  ...
  subgraph BATCH["批内：向量聚类或冷启动"]
    ...
    C2 --> C3["内部 _merge_soft_policy_clusters"]
    ...
    V4 --> V5["_merge_soft_policy_clusters<br/>重要 topic + 禁飞禁售关键词<br/>+ CLUSTER_MERGE_HOURS 同窗"]
...
| 冷启动全局去重、按 topic 成簇、软合并、簇内去重、排序 | `src/pipeline.py`：`_global_dedupe_prepared`、`_clusters_from_prepared`、`_merge_soft_policy_clusters`、`_dedupe_within_cluster`、`_order_clusters_for_publish` |
| `run_once` 中分支与统计 | `src/pipeline.py`：`emb_clusters = await build_embedding_event_clusters(...)` 一段 |

**冷启动与向量路径的差异**：冷启动路径里 `_merge_soft_policy_clusters` 在 `_clusters_from_prepared` **内部**已调用；向量路径在 `build_embedding_event_clusters` **之后**再调用一次 `_merge_soft_policy_clusters`，用于把「同窗 + 政策关键词命中」的重要簇做软合并。

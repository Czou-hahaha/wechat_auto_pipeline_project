...
PLATFORM_BASE = os.environ.get("PLATFORM_BASE", "http://127.0.0.1:3001").rstrip("/")
BFF_BASE = os.environ.get("BFF_BASE", "http://127.0.0.1:8788").rstrip("/")
...
def execute(c: Checker, skip_mutations: bool, test_wechat_push: bool) -> int:
    ...
    if items:
        ...
        try:
            ...
            if not event_id:
                ...
                if evs:
                    event_id = str(evs[0].get("id") or "")
        except Exception:
            pass
        if event_id:
            c.ok("Events 列表 seed 过滤", f"items=0，详情回退 event_id={event_id[:8]}…")
        else:
            c.fail("Events 非空", "items=0 且无 events.json 可回退")
    ...
    c.http("通知", "GET", f"{base}/api/notifications")
    c.http("阶段耗时", "GET", f"{base}/api/pipeline/phase-timings")
    c.http("反馈汇总", "GET", f"{base}/api/feedback/summary")
    ...
    if skip_mutations:
        if event_id:
            ...
            c.http("事件图谱 GET", "GET", f"{base}/api/events/{event_id}/graph")
...
def main() -> int:
    ...
    if failed:
        print("\n失败项请检查 BFF :8788 与 Platform :3001")
    print("\n--- UI → API 对照 ---")

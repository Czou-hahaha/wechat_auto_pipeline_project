...
def _event_title(event: dict[str, Any]) -> str:
    return str(
        event.get("headline_zh") or event.get("title_zh") or event.get("title") or ""
    ).strip()


# 标题相似度过高视为同题重复稿，不作为「关联事件」展示（避免入库重复 event 进图谱）
_NEAR_DUPLICATE_TITLE_SIM = 0.82
...
def build_event_history_payload(store: JsonStore, event_id: str) -> dict[str, Any]:
    ...
    for row in store.list_events():
        ...
        if sim >= _NEAR_DUPLICATE_TITLE_SIM:
            continue

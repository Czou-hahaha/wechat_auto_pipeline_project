...
_EXTRACT_LINKS_JS = """
(args) => {
  const hrefNeedle = (args && args.hrefNeedle) || '';
  const timeSelector = (args && args.timeSelector) || '';
  ...
  for (const a of document.querySelectorAll('a[href]')) {
    ...
    seen.add(href);
    let ctx = '';
    const p = a.closest('div, li, article, section, .kr-search-article-item, .article-item');
    if (p) ctx = (p.innerText || '').slice(0, 400);
    let timeText = '';
    if (timeSelector && p) {
      const tnode = p.querySelector(timeSelector);
      if (tnode) timeText = (tnode.innerText || tnode.textContent || '').replace(/\\s+/g, ' ').trim();
    }
    if (!timeText && timeSelector) {
      const g = a.parentElement && a.parentElement.querySelector(timeSelector);
      if (g) timeText = (g.innerText || g.textContent || '').replace(/\\s+/g, ' ').trim();
    }
    out.push({ href, title, ctx, timeText });
...
async def _collect_from_page(page, *, strategy: dict, domain: str, now: datetime, max_hours: int) -> list[tuple[str, str, str]]:
    ...
    time_selector = str(strategy.get("time_selector") or "").strip()
    ...
    try:
        rows = await page.evaluate(
            _EXTRACT_LINKS_JS,
            {"hrefNeedle": needle, "timeSelector": time_selector},
        )
    except Exception:
    ...
    for row in rows:
        ...
        time_text = str(row.get("timeText") or "").strip()
        if not href or not _host_ok(href, domain):
            continue
        pub_raw = time_text or ctx
        pub = _published_at_for_browser_hit(
            _parse_context_datetime(pub_raw, now=now),
            max_hours=max_hours,
            now=now,
        )

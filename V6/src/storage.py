...
class ArticleRecord:
    ...
    event_id: str = ""
    summary_zh: str = ""
    # 事件扩搜入库时的 bge-m3 向量相似度（相对种子稿 bank）；0 表示未记录
    expansion_similarity: float = 0.0
...
class JsonStore:
    ...
    def map_entries_for_event(self, event_id: str) -> list[dict]:
        """``event_id`` 在 ``event_article_map.json`` 中的全部映射行。"""
        eid = (event_id or "").strip()
        if not eid:
            return []
        return [
            m
            for m in self._read_event_map()
            if isinstance(m, dict) and str(m.get("event_id") or "").strip() == eid
        ]
    ...
    def count_wechat_draft_pushed_events_on_local_date(
        self,
        local_date: date,
        *,
        ...
        """
        按本地自然日统计已推微信草稿的「事件」数。

        - ``events.json`` 的 ``event_wechat_draft_pushed_at`` 每个事件至多计 1；
        - 仅文章表有 ``wechat_draft_pushed_at``、事件表未打标时，按 ``event_id`` 去重；
        - 无 ``event_id`` 的孤立稿按 ``article id`` 各计 1（兼容 ``push-today-drafts``）。
        """
        tz = ZoneInfo((timezone or "Asia/Shanghai").strip())
        seen: set[str] = set()

        for ev in self._read_events():
            if not isinstance(ev, dict):
                continue
            pushed = self._parse_iso_datetime(
                str(ev.get("event_wechat_draft_pushed_at") or "")
            )
            if pushed is None or pushed.astimezone(tz).date() != local_date:
                continue
        ...
        for row in self._read():
            if not isinstance(row, dict):
                continue
            pushed = self._parse_iso_datetime(
                str(row.get("wechat_draft_pushed_at") or "")
            )
            if pushed is None or pushed.astimezone(tz).date() != local_date:
                continue
            ...
            if aid:
                seen.add(f"article:{aid}")

        return len(seen)

    def sync_event_draft_pushed_flags(self) -> int:
        """
        将各 event 的 ``event_wechat_draft_pushed_at`` 与关联稿
        ``wechat_draft_pushed_at`` 对齐（取最新时间戳）。返回写入事件条数。
        """
        ...
        for row in self._read():
            if not isinstance(row, dict):
                continue
            eid = str(row.get("event_id") or "").strip()
            ts = str(row.get("wechat_draft_pushed_at") or "").strip()
            if eid and ts:
                by_event.setdefault(eid, []).append(ts)
        updated = 0
        for row in events:
            if not isinstance(row, dict):
                continue
            ...
            if not eid:
                continue
            stamps: list[str] = []
            ev_ts = str(row.get("event_wechat_draft_pushed_at") or "").strip()
            if ev_ts:
                stamps.append(ev_ts)
            stamps.extend(by_event.get(eid, []))
            if not stamps:
                continue
            best = max(stamps)
            if ev_ts != best:
                row["event_wechat_draft_pushed_at"] = best
                updated += 1
        if updated:

...
from src.bff.event_mapper import event_to_list_item, event_visible_in_list
...
def build_event_list_items(store: JsonStore) -> list[dict[str, Any]]:
    ...
    for ev in store.list_events():
        ...
        roles = roles_by_event.get(eid, {})
        if not event_visible_in_list(ev, articles, url_roles=roles):
            continue
        out.append(
            event_to_list_item(
                ev,

...
def _linked_article_count(
    articles: list[dict[str, Any]], url_roles: dict[str, str] | None = None
) -> int:
    """articles.json 行数与 event_article_map 映射条数取较大值（瘦身/清空后仍可用）。"""
    return max(_event_article_count(articles), len(url_roles or {}))


def _map_support_count(url_roles: dict[str, str] | None) -> int:
    return sum(
        1 for r in (url_roles or {}).values() if str(r or "").strip().lower() == "support"
    )
...
def _has_event_news_press(
    ...
    *,
    ...
    if _linked_article_count(articles, url_roles) < need:
        return False
...
def _expansion_article_count(
    ...
    return max(row_n, _map_support_count(url_roles))


def _map_primary_count(url_roles: dict[str, str] | None) -> int:
    """event_article_map 中 role=primary 的条数（articles.json 瘦身/清空后仍可作为主稿依据）。"""
    return sum(
        1 for r in (url_roles or {}).values() if str(r or "").strip().lower() == "primary"
    )


def _seed_article_count(articles: list[dict[str, Any]], url_roles: dict[str, str] | None = None) -> int:
    roles = url_roles or {}
    n = sum(
        1
        for a in articles
        if isinstance(a, dict)
        and (
            _article_source_kind(a, roles) == "seed"
            or str(a.get("status") or "").strip().lower() == "ready_for_review"
        )
    )
    if n >= 1:
        return n
    return _map_primary_count(roles)
...
def _needs_expansion_tag(
    ...
    *,
    ...
    """列表「待扩搜」= 有主稿（seed≥1）、尚无扩搜稿、且未达可推送。"""
    if pushable:
        return False
    if _expansion_article_count(articles, url_roles) > 0:
        return False
    if _seed_article_count(articles, url_roles) < 1:
        return False
    return True
...
def _event_list_tags(
    ...
    *,
    ...
    if pushable:
        return ["可推送"]
    if _needs_expansion_tag(event, articles, pushable=False, url_roles=url_roles):
        return ["待扩搜"]
    return []
...
def _no_summary_reason(
    ...
    *,
    ...
    if has_summary:
        return ""
    n = _linked_article_count(articles, url_roles)
    if n < _min_articles_for_cluster_summary():
        return "稿数不足"
    return "待生成通稿"
...
def _enhancement_list_meta(
    ...
    *,
    ...
    inserted = _expansion_article_count(articles, url_roles)
    if not enhancement_row:
...
def event_visible_in_list(
    ...
    *,
    ...
    """V6 列表：无主稿事件不展示（seed≥1，或已有通稿且库内有参考稿）。"""
    try:
        from src.config import Settings

        if not bool(getattr(Settings(), "v6_event_list_require_seed_article", True)):
            return True
    except Exception:
        pass
    roles = url_roles or {}
    seed = _seed_article_count(articles, roles)
    if seed >= 1:
        return True
    map_primary = _map_primary_count(roles)
    if map_primary >= 1:
        return True
    if _event_press_text(event) and str(event.get("event_press_generated_at") or "").strip():
        if map_primary >= 1 or _reference_article_count(articles) >= 1:
            return True
    if has_summary is None:
        has_summary = _has_event_news_press(event, articles, url_roles=roles)
    if has_summary and (_reference_article_count(articles) >= 1 or map_primary >= 1):
        return True
    return False
...
def event_to_list_item(
    ...
    *,
    ...
    has_summary = _has_event_news_press(event, articles, url_roles=roles)
    tags = _event_list_tags(event, articles, has_summary=has_summary, url_roles=roles)
    ...
    enh_status, expansion_count = _enhancement_list_meta(
        enhancement_row, articles, url_roles=roles
    )
    seed_count = _seed_article_count(articles, roles)
    linked_count = _linked_article_count(articles, roles)
    countries = sorted(
    ...
    return {
        ...
        "noSummaryReason": _no_summary_reason(
            event, articles, has_summary=has_summary, url_roles=roles
        ),
        "wechatDraftPushedAt": _wechat_draft_pushed_at(event, articles),
        "importance_score": _importance_score(event, articles),
        "qa_score": int(event.get("event_press_qa_score") or 0),
        "article_count": linked_count,

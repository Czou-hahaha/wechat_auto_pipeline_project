...
async def push_event_press_to_wechat(
    ...
    *,
    ...
    from src.bff.event_list_cache import invalidate_event_list_cache
...
def mark_event_draft_pushed(
    ...
    *,
    ...
    """
    仅写入「已入草稿箱」时间戳，不调用微信 API。
    用于在公众平台手工推草稿后，与列表「草稿已推」分段对齐。
    """
    ev = store.get_event(event_id)
    if not ev:
        raise ValueError("event not found")
    ts = (pushed_at or "").strip() or datetime.now(timezone.utc).isoformat()
    store.patch_event(event_id, {"event_wechat_draft_pushed_at": ts})
    for a in store.articles_for_event(event_id):
        aid = str(a.get("id") or "").strip()
        if aid:
            store.patch_article(aid, {"wechat_draft_pushed_at": ts})
    from src.bff.event_list_cache import invalidate_event_list_cache

    invalidate_event_list_cache()
    logger.info("event draft marked pushed event_id=%s", event_id[:13])
    return {"ok": True, "pushedAt": ts, "markedOnly": True}


def mark_event_draft_pushed_sync(
    store: JsonStore,
    event_id: str,
    *,
    pushed_at: str | None = None,
) -> dict[str, Any]:
    return mark_event_draft_pushed(store, event_id, pushed_at=pushed_at)

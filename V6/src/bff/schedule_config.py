...
def put_schedule_config(body: dict[str, Any]) -> dict[str, Any]:
    ...
    from src.bff.scheduler_run import read_status, sync_scheduler, stop_scheduler
    from src.config import Settings
    ...
    if not settings.bff_scheduler_autostart:
        ...
        out["scheduler"] = read_status(data_dir)
        out["schedulerNote"] = (
            "V6 开发轨不启动定时调度；每日 8:00/20:00 采集请在 V5 设置页配置并保存。"
        )
        return out
    sync = sync_scheduler(
        data_dir,

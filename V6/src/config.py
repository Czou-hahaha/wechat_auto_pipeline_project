...
class Settings(BaseSettings):
    ...
    # 中文站 Playwright 站内搜索（仅 browser_zh_sources.json；外国站不走此路径）
    browser_zh_search_enabled: bool = Field(default=False, alias="BROWSER_ZH_SEARCH_ENABLED")
    browser_zh_sources_path: str = Field(
        default="config/browser_zh_sources.json",
        alias="BROWSER_ZH_SOURCES_PATH",
    )
    browser_zh_keywords_path: str = Field(
        default="config/browser_zh_keywords.json",
        alias="BROWSER_ZH_KEYWORDS_PATH",
    )
    browser_zh_max_age_hours: int = Field(default=24, ge=1, le=168, alias="BROWSER_ZH_MAX_AGE_HOURS")
    browser_zh_max_keywords: int = Field(default=3, ge=1, le=20, alias="BROWSER_ZH_MAX_KEYWORDS")
    browser_zh_per_site_max: int = Field(default=25, ge=5, le=100, alias="BROWSER_ZH_PER_SITE_MAX")
    browser_zh_total_max: int = Field(default=200, ge=20, le=2000, alias="BROWSER_ZH_TOTAL_MAX")
    browser_zh_headless: bool = Field(default=True, alias="BROWSER_ZH_HEADLESS")
    browser_zh_nav_timeout_sec: float = Field(default=35.0, ge=10.0, le=120.0, alias="BROWSER_ZH_NAV_TIMEOUT_SEC")
    # true: 中文站检索优先使用 Playwright；命中不足时回退 html_list/rss
    browser_zh_primary_mode: bool = Field(default=True, alias="BROWSER_ZH_PRIMARY_MODE")
    browser_zh_min_hits_for_primary: int = Field(
        default=20, ge=1, le=500, alias="BROWSER_ZH_MIN_HITS_FOR_PRIMARY"
    )
    ...
    # 中文电报/快讯稿：正文常 <400 字；入库门槛与 trafilatura 下限单独配置
    zh_ingest_min_chars: int = Field(default=80, ge=20, le=400, alias="ZH_INGEST_MIN_CHARS")
    en_ingest_min_chars: int = Field(default=400, ge=100, le=2000, alias="EN_INGEST_MIN_CHARS")
    zh_trafilatura_min_chars: int = Field(default=80, ge=20, le=400, alias="ZH_TRAFILATURA_MIN_CHARS")
    # true：中文站跳过关键词相关性，改由 DeepSeek 判定是否低空/政策/前沿科技
    zh_domain_relevance_ai_enabled: bool = Field(
        default=True, alias="ZH_DOMAIN_RELEVANCE_AI_ENABLED"
    )
    # true：中文源走升级入库路径（跳过 prefilter 关键词/字数门槛，抓正文后 LLM 门禁 + 聚类后 LLM 事件提炼）
    zh_ingest_llm_enabled: bool = Field(default=False, alias="ZH_INGEST_LLM_ENABLED")
    en_ingest_llm_enabled: bool = Field(default=True, alias="EN_INGEST_LLM_ENABLED")
    v6_event_list_require_seed_article: bool = Field(
        default=True, alias="V6_EVENT_LIST_REQUIRE_SEED_ARTICLE"
    )
    bff_port: int = Field(default=8788, ge=1024, le=65535, alias="BFF_PORT")
    # true：中文源不因字数过短拦截（与 ZH_INGEST_LLM_ENABLED 配合；单独开启时仅跳过字数）
    zh_skip_length_gate: bool = Field(default=True, alias="ZH_SKIP_LENGTH_GATE")
    zh_llm_article_concurrency: int = Field(default=3, ge=1, le=8, alias="ZH_LLM_ARTICLE_CONCURRENCY")
    zh_llm_event_extract_max_chars: int = Field(
        default=24, ge=12, le=40, alias="ZH_LLM_EVENT_EXTRACT_MAX_CHARS"
    )
    ...
    # 反馈策略覆盖文件（由 feedback policy 引擎生成并手工确认应用）
    feedback_policy_overrides_path: str = Field(
        default="config/feedback_policy_overrides.json",
        alias="FEEDBACK_POLICY_OVERRIDES_PATH",
    )

    # ----- V4：Event Graph Memory（时序事件记忆层） -----
    event_graph_enabled: bool = Field(default=True, alias="EVENT_GRAPH_ENABLED")
    event_graph_database_url: str = Field(default="", alias="EVENT_GRAPH_DATABASE_URL")
    event_graph_config_path: str = Field(default="config/event_graph.json", alias="EVENT_GRAPH_CONFIG_PATH")

    # ----- 数据源健康探测 -----
    data_source_health_timeout_sec: float = Field(
        default=25.0, ge=8.0, le=120.0, alias="DATA_SOURCE_HEALTH_TIMEOUT_SEC"
    )
    data_source_health_min_samples: int = Field(
        default=1, ge=1, le=20, alias="DATA_SOURCE_HEALTH_MIN_SAMPLES"
    )
    data_source_health_weekly_enabled: bool = Field(
        default=True, alias="DATA_SOURCE_HEALTH_WEEKLY_ENABLED"
    )
    data_source_health_weekly_dow: str = Field(
        default="mon", alias="DATA_SOURCE_HEALTH_WEEKLY_DOW"
    )
    data_source_health_weekly_hour: int = Field(
        default=4, ge=0, le=23, alias="DATA_SOURCE_HEALTH_WEEKLY_HOUR"
    )
    data_source_health_weekly_minute: int = Field(
        default=0, ge=0, le=59, alias="DATA_SOURCE_HEALTH_WEEKLY_MINUTE"
    )

    data_dir: str = Field(default="./data", alias="DATA_DIR")
    log_level: str = Field(default="INFO", alias="LOG_LEVEL")
    # V6 开发轨默认 false；生产定时仅 V5 跑 run-scheduler
    bff_scheduler_autostart: bool = Field(
        default=False, alias="BFF_SCHEDULER_AUTOSTART"
    )
    ...
    def parsed_browser_zh_keywords(self) -> list[str]:
        """Playwright 中文站内搜索专用词表（``browser_zh_keywords.json``），与 RSS/GDELT 词表分离。"""
        raw = (self.browser_zh_keywords_path or "config/browser_zh_keywords.json").strip()
        path = Path(raw)
    ...
    def feedback_policy_overrides(self) -> dict[str, object]:
        raw = (self.feedback_policy_overrides_path or "").strip()
        if not raw:
            return {}
        p = Path(raw).expanduser()
        if not p.is_absolute():
        ...
        try:
            ...
            logger.warning("failed to load feedback policy overrides: %s", p, exc_info=True)
            return {}

    def effective_qa_rewrite_pass_threshold(self) -> int:
        ov = self.feedback_policy_overrides().get("qa_rewrite_pass_threshold")
        try:
            val = int(ov) if ov is not None else int(self.qa_rewrite_pass_threshold)
        except Exception:
            val = int(self.qa_rewrite_pass_threshold)
        return max(0, min(100, val))

    def effective_qa_rewrite_max_rounds(self) -> int:
        ov = self.feedback_policy_overrides().get("qa_rewrite_max_rounds")
        try:
            val = int(ov) if ov is not None else int(self.qa_rewrite_max_rounds)
        except Exception:
            val = int(self.qa_rewrite_max_rounds)
        return max(0, min(8, val))

    def effective_search_prefilter_enabled(self) -> bool:
        ov = self.feedback_policy_overrides().get("search_prefilter_enabled")
        if ov is None:
            return bool(self.search_prefilter_enabled)
        return bool(ov)

    def wechat_ready(self) -> bool:
        return bool(self.wechat_mp_app_id.strip() and self.wechat_mp_app_secret.strip())

    def zh_media_host_set(self) -> frozenset[str]:
        """config/data_sources.json 中 language=zh 的站点域名。"""
        path = Path((self.his_data_sources_path or "config/data_sources.json").strip()).expanduser()
        if not path.is_absolute():
            path = Path.cwd() / path
        hosts: set[str] = set()
        if not path.is_file():
        ...
        for row in rows:
            if not isinstance(row, dict):
                continue
            lang = str(row.get("language") or "zh").strip().lower()
            if lang not in ("zh", "chinese", "cn"):
                continue
            value = str(row.get("value") or "").strip().lower()
            if not value:
                continue
            site = value.replace("https://", "").replace("http://", "").strip("/")
            if "/" in site:
                site = site.split("/", 1)[0]
            if site.startswith("www."):
                site = site[4:]

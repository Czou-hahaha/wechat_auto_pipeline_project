...
async def run_event_press_generation(
    ...
    for ev in events:
        ...
        headline_zh = ""
        display_title = title
        if bool(getattr(settings, "event_translate_summary_enabled", True)):
            try:
                ...
                title_zh_new, display_title, body = await ensure_press_title_body_zh(
                    summarizer, title=title, body=body
                )
            except Exception:
                logger.warning(
                    "event press zh guard failed event=%s",
                    eid[:13],
                    exc_info=True,
                )

        if qa_patch.get("event_press_qa_approved") or int(
            qa_patch.get("event_press_qa_score") or 0
        ) >= int(settings.effective_qa_rewrite_pass_threshold()):
            try:
                from src.ai import SummaryService
                from src.utils.headline_zh import first_zh_sentence
                ...
                headline_zh = await summarizer.generate_compact_headline_zh(
                    context_title=title,
                    press_lede=first_zh_sentence(body),
                )
                if headline_zh:
                    title_zh_new = headline_zh
            except Exception:
                logger.warning(
                    "compact headline failed event=%s",
                    eid[:13],
                    exc_info=True,
                )
        ...
        if headline_zh:
            patch["headline_zh"] = headline_zh
        if title_zh_new:
            patch["title_zh"] = title_zh_new

...
from src.services.zh_ingest import assess_en_article, assess_zh_article, extract_zh_cluster_event
from src.utils.zh_ingest_audit import (
    flush_zh_ingest_audit,
    record_zh_ingest_audit,
    reset_zh_ingest_audit,
)
from src.services.zh_ingest.event_extract import ZhClusterExtractResult
...
from src.utils.topic_prefilter import (
    ...
    latin_low_altitude_keyword_hit,
...
class PipelineRunner:
    def __init__(self, settings: Settings):
        ...
        self._zh_media_hosts = settings.zh_media_host_set()
        self._phase_timings = None

    def _phase_mark(self, name: str) -> None:
        from src.utils.phase_timings import PhaseTimings

        if self._phase_timings is None:
            self._phase_timings = PhaseTimings()
            self._phase_timings.mark("ingest")
        self._phase_timings.mark(name)

    def flush_phase_timings(self) -> None:
        if self._phase_timings is None:
            return
        import json

        payload = self._phase_timings.finish()
        path = self.store.data_dir / "phase_timings_last_run.json"
    ...
    def _cluster_passes_editorial(cluster: list[PreparedArticle]) -> bool:
        ...
        return should_keep_editorial_content(
            ...
            text=primary.text[:EDITORIAL_FULL_TEXT_MAX],
            url=primary.hit.url or primary.final_url or "",
        )

    def _cluster_passes_editorial_for_run(self, cluster: list[PreparedArticle]) -> bool:
        if not cluster:
            return False
        if bool(self.settings.zh_ingest_llm_enabled) and self._cluster_is_zh_dominant(cluster):
            return True
        return self._cluster_passes_editorial(cluster)
    ...
    async def _run_mandatory_event_enhancement_post_pipeline(self) -> None:
        ...
        await self._run_event_graph_memory()

    async def _run_event_graph_memory(self) -> None:
        """V4：Event Graph Memory — 实体/关系/时间线/演化追踪。"""
        if not self.settings.event_graph_enabled:
            return
        try:
            from src.services.event_memory import EventMemoryService

            svc = EventMemoryService(self.settings)
            stats = await svc.process_events_from_store(self.store)
            logger.info("event graph memory done stats=%s", stats)
        except Exception:
            logger.exception("event graph memory failed (ignored)")
    ...
    async def run_once(self) -> RunStats:
        ...
        skipped_pf = 0
        if self.settings.effective_search_prefilter_enabled():
            kept: list[SearchHit] = []
            prefilter_fn = (
                should_fetch_zh_media_hit
            ...
            for hit in candidates:
                if (
                    bool(self.settings.zh_ingest_llm_enabled)
                    and self._host_is_zh_media(hit.url)
                ):
                    probe_title = normalize_title((hit.title or "").strip())
                    if self._is_low_signal_digest_column(probe_title, hit.snippet or ""):
                        skipped_pf += 1
                        logger.info("skip digest/column (zh llm pre-fetch): %s", hit.url)
                        continue
                    kept.append(hit)
                elif prefilter_fn(hit):
                    kept.append(hit)
                else:
        ...
        for hit in candidates:
            try:
                ...
                if self._is_low_signal_digest_column(title, text[:800]):
                    ...
                    continue
                page_url = final_url or hit.url
                use_zh_llm = self._uses_zh_llm_ingest(page_url)
                use_en_llm = (
                    not use_zh_llm and bool(getattr(self.settings, "en_ingest_llm_enabled", True))
                )
                if use_zh_llm:
                    if not await self._passes_zh_llm_ingest_scope_gate(
                        title=title,
                        text=text,
                        snippet=hit.snippet,
                        source_url=page_url,
                    ...
                    if not await self._passes_en_llm_ingest_scope_gate(
                        title=title,
                        text=text,
                        snippet=hit.snippet,
                        source_url=page_url,
                    ...
                    if not await self._passes_ingest_scope_gate(
                        title=title,
                        text=text,
                        snippet=hit.snippet,
                        source_url=page_url,
                    ...
                    if not should_keep_editorial_content(
                        ...
                        text=text[:EDITORIAL_FULL_TEXT_MAX],
                        url=page_url,
                    ):
                        stats.skipped_policy += 1
                ...
                if self.store.exists_duplicate(hit.url, title, text, landing_url=final_url):
                    stats.skipped_duplicate += 1
                    continue
                editorial_blob = self._editorial_blob_for_length(
                    title=title, snippet=hit.snippet or "", text=text
                )
                min_body = self._min_ingest_body_chars(final_url or hit.url)
                skip_length = use_zh_llm and bool(self.settings.zh_skip_length_gate)
                if not skip_length and self._is_too_short(editorial_blob, min_chars=min_body):
                    stats.skipped_too_short += 1
                    logger.info(
                        "skip too short article (<%d chars, zh=%s): %s",
                        min_body,
                        self._host_is_zh_media(final_url or hit.url),
                        hit.url,
                    )
                    continue
        ...
        for c in clusters_merged:
            ...
            if d and self._cluster_passes_editorial_for_run(d):
                clusters_deduped.append(d)

        cluster_llm_meta: dict[int, ZhClusterExtractResult] = {}
        if bool(self.settings.zh_ingest_llm_enabled):
            clusters_after_extract: list[list[PreparedArticle]] = []
            for c in clusters_deduped:
                if not self._cluster_is_zh_dominant(c):
                    clusters_after_extract.append(c)
                    continue
                meta = await extract_zh_cluster_event(
                    self.summarizer, self.settings, c
                )
                if not meta.keep_cluster:
                    logger.info(
                        "zh cluster extract rejected: reason=%s size=%d primary=%s",
                        meta.reject_reason,
                        len(c),
                        c[0].title[:60] if c else "",
                    )
                    continue
                cluster_llm_meta[id(c)] = meta
                logger.info(
                    "zh cluster extract ok: title_zh=%s size=%d",
                    meta.event_title_zh,
                    len(c),
                )
                clusters_after_extract.append(c)
            clusters_deduped = clusters_after_extract
        ...
        async def _seed_cluster(cluster: list[PreparedArticle]) -> dict[str, str | int]:
            delta: dict[str, str | int] = {"event_id": "", "failed": 0, "accumulated": 0}
            try:
                deduped = cluster
                ...
                existing_id = await match_existing_event_id(
                    cluster=deduped,
                    store=self.store,
                    settings=self.settings,
                )
                event_id = existing_id or str(uuid4())
                is_accumulate = bool(existing_id)
                ...
                llm_meta = cluster_llm_meta.get(id(deduped))
                if llm_meta and llm_meta.event_title_zh:
                    event_title = llm_meta.event_title_zh
                    preset_title_zh = llm_meta.event_title_zh
                elif event_lang == "en":
                    ...
                    event_title = primary.title
                    preset_title_zh = ""
                if llm_meta and llm_meta.event_title_en and event_lang == "en":
                    event_title = llm_meta.event_title_en or event_title
                known_urls = urls_already_in_event(self.store, event_id) if is_accumulate else set()
                async with store_lock:
                    ...
                    for p in deduped:
                        ...
                        fp = JsonStore.url_fingerprint(p.final_url or p.hit.url)
                        if fp and fp in known_urls:
                            continue
                        if fp:
                            known_urls.add(fp)
                        self.store.add(
                        ...
                        map_entries.append(
                            {
                                ...
                                "role": "support" if is_accumulate else ("primary" if p is primary else "source"),
                            }
                        )
                    if not is_accumulate:
                        seed_title_zh = preset_title_zh
                        if not seed_title_zh and self.summarizer._text_is_mostly_non_cjk(event_title):
                            try:
                                seed_title_zh = (
                                    await self.summarizer.translate_title_to_zh(
                                        title=event_title
                                    )
                                )
                            except Exception:
                                logger.warning(
                                    "seed title_zh failed event=%s",
                                    event_id[:13],
                                    exc_info=True,
                                )
                        self.store.append_event(
                            EventRecord(
                                ...
                                summary="",
                                summary_zh="",
                                title_zh=seed_title_zh,
                                dominant_topic_key=dom_key or primary.topic_key,
                                created_at=now_iso,
                            )
                        )
                    if map_entries:
                        self.store.append_event_article_map(map_entries)
                    if is_accumulate:
                        delta["accumulated"] = added
                logger.info(
                    "%s event_id=%s members=%d new_articles=%d title=%s",
                    "event accumulated into existing"
                    if is_accumulate
                    else "event seeded (summary after expansion)",
                    event_id[:13],
                    len(deduped),
                    int(delta.get("accumulated", 0) or len(deduped)),
                    primary.title[:80],
                )
        ...
        self._phase_mark("ingest_cluster")
        await self._run_mandatory_event_enhancement_post_pipeline()
        self._phase_mark("enhancement_press_graph")
        ...
        self._phase_mark("done")
        self.flush_phase_timings()
        flush_zh_ingest_audit(self.settings.data_path())
        return stats
        ...
        async def _one(event_id: str) -> None:
            ...
            async with sem:
                ...
                if len(usable) < min_articles:
                    ...
                    ev_row = self.store.get_event(event_id) or {}
                    raw_title = str(ev_row.get("title") or "").strip()
                    if raw_title and self.summarizer._text_is_mostly_non_cjk(raw_title):
                        try:
                            tzh = await self.summarizer.translate_title_to_zh(title=raw_title)
                            if tzh:
                                self.store.patch_event(
                                    event_id, {"title_zh": tzh}
                                )
                        except Exception:
                            logger.warning(
                                "title_zh backfill failed event=%s",
                                event_id[:13],
                                exc_info=True,
                            )
                    return
                ...
                title_zh_out = (tzh or "").strip()
                if title_zh_out:
                    title_zh_out = trim_zh_headline_heuristic(title_zh_out)
                elif self.summarizer._cjk_ratio(display_title) >= 0.2:
                    title_zh_out = trim_zh_headline_heuristic(display_title.strip())
                if not title_zh_out and self.summarizer._text_is_mostly_non_cjk(draft_title):
                    try:
                        title_zh_out = trim_zh_headline_heuristic(
                            (
                                await self.summarizer.translate_title_to_zh(
                                    title=draft_title
                                )
                        ...
                        logger.warning(
                            "title_zh after cluster summary failed event=%s",
                            event_id[:13],
                            exc_info=True,
                        )
                event_patch = {
                    "title": draft_title,
                    "summary": summary,
                    "summary_zh": summary_zh or display_summary,
                    "title_zh": title_zh_out,
                }
                if wechat_draft_pushed_at:
                    event_patch["event_wechat_draft_pushed_at"] = wechat_draft_pushed_at
                self.store.patch_event(event_id, event_patch)
                if primary_id:
        ...
        for row in candidates:
            ...
            try:
                ...
                if aid:
                    ...
                    if eid:
                        self.store.patch_event(
                            eid, {"event_wechat_draft_pushed_at": pushed_at}
                        )
    ...
    def _remaining_daily_publish_slots(self) -> int:
        """
        按 ``SCHEDULE_TIMEZONE`` 本地自然日统计**已推送事件数**（非关联文章条数），
        与 ``MAX_PUBLISH_PER_DAY`` 比较得到剩余配额。
        """
        cap = max(0, int(self.settings.max_publish_per_day))
        if cap <= 0:
            return 0
        tz_name = self.settings.schedule_timezone.strip() or "Asia/Shanghai"
        today = datetime.now(ZoneInfo(tz_name)).date()
        pushed_today = self.store.count_wechat_draft_pushed_events_on_local_date(
            today,
            timezone=tz_name,
        )
    ...
    def _extract_published_at(self, soup: BeautifulSoup, *, page_url: str) -> tuple[str, bool]:
        ...
        if "36kr.com" in host:
            ...
            for sel in (
                "span.item-time",
                ".article-title-icon span.item-time",
                "span.title-icon-item.item-time",
            ):
            ...
            kr_candidates.extend(self._script_embedded_time_candidates(soup))
            for raw in kr_candidates:
    ...
    def _host_is_zh_media(self, url: str) -> bool:
        if bool(self.settings.ingest_zh_html_only):
            return True
        try:
        ...
        if not host:
            return False
        if host in self._zh_media_hosts:
            return True
        return any(host == h or host.endswith("." + h) for h in self._zh_media_hosts)

    def _uses_zh_llm_ingest(self, url: str) -> bool:
        return bool(self.settings.zh_ingest_llm_enabled) and self._host_is_zh_media(url)

    def _cluster_is_zh_dominant(self, cluster: list[PreparedArticle]) -> bool:
        if not cluster:
            return False
        zh_n = sum(
            1
            for p in cluster
            if self._host_is_zh_media(p.final_url or p.hit.url or "")
        )
        return zh_n >= 1

    async def _passes_zh_llm_ingest_scope_gate(
        self, *, title: str, text: str, snippet: str, source_url: str
    ) -> bool:
        if self._is_scope_excluded(title=title, text=text, source_url=source_url):
            logger.info("skip scope-excluded page (zh llm): %s", source_url)
            record_zh_ingest_audit(
                url=source_url, title=title, stage="scope_excluded", keep=False, reason="scope"
            )
            return False
        if self._is_opinion_or_non_news(title=title, text=text):
            logger.info("skip opinion/non-news (zh llm): %s", source_url)
            record_zh_ingest_audit(
                url=source_url, title=title, stage="opinion", keep=False, reason="opinion"
            )
            return False
        gate = await assess_zh_article(
            self.summarizer,
            self.settings,
            title=title,
            text=text,
        ...
        if not gate.keep:
            logger.info(
                "skip zh LLM article gate: %s category=%s reason=%s political=%s smear=%s",
                source_url,
                gate.category,
                gate.reason,
                gate.is_political_sensitive,
                gate.is_anti_china_smear,
            )
            record_zh_ingest_audit(
                url=source_url,
                title=title,
    ...
    async def _passes_en_llm_ingest_scope_gate(
        self, *, title: str, text: str, snippet: str, source_url: str
    ) -> bool:
        if self._is_news_brief_or_roundup(title=title, text=text, snippet=snippet):
            logger.info("skip quick-news/roundup page (en llm): %s", source_url)
            return False
        if self._is_scope_excluded(title=title, text=text, source_url=source_url):
            logger.info("skip scope-excluded page (en llm): %s", source_url)
            return False
        if self._is_opinion_or_non_news(title=title, text=text):
            logger.info("skip opinion/non-news (en llm): %s", source_url)
            return False
        ...
        if not gate.keep:
            logger.info(
                "skip en LLM article gate: %s category=%s reason=%s",
                source_url,
                gate.category,
                gate.reason,
            )
    ...
    def _min_ingest_body_chars(self, url: str) -> int:
        if self._host_is_zh_media(url):
            return int(self.settings.zh_ingest_min_chars)
        return int(self.settings.en_ingest_min_chars)

    @staticmethod
    def _editorial_blob_for_length(*, title: str, snippet: str, text: str) -> str:
        """电报体正文偏短时，将标题+摘要并入字数统计。"""
        parts = [(title or "").strip(), (snippet or "").strip(), (text or "").strip()]
        return "\n".join(p for p in parts if p)

    async def _passes_ingest_scope_gate(
        self, *, title: str, text: str, snippet: str, source_url: str
    ) -> bool:
        if self._is_news_brief_or_roundup(title=title, text=text, snippet=snippet):
        ...
        if is_war_conflict_content(title=title, snippet=snippet, text=text):
            logger.info("skip war/conflict page (full-text gate): %s", source_url)
            return False
        zh = self._host_is_zh_media(source_url)
        if zh and bool(self.settings.zh_domain_relevance_ai_enabled):
            rel = await self.summarizer.assess_domain_relevance(
                title=title, text=text, snippet=snippet
            )
            if not rel.get("is_relevant"):
                logger.info(
                    "skip zh AI domain gate: %s category=%s reason=%s",
                    source_url,
                    rel.get("category"),
                    rel.get("reason"),
                )
            ...
            if not self._is_low_altitude_relevant(title=title, text=text, snippet=snippet):
                logger.info("skip low-relevance non low-altitude page: %s", source_url)
                return False
            if not self._is_low_altitude_primary_focus(title=title, text=text, snippet=snippet):
    ...
    def _passes_scope_gate(cls, *, title: str, text: str, snippet: str, source_url: str) -> bool:
        ...
        if is_vocational_education_offtopic(title=title, snippet=snippet, text=text):
            logger.info("skip vocational/skills off-topic page: %s", source_url)
            return False
    ...
    async def _fetch_text(
        ...
        traf_min = int(self.settings.trafilatura_min_chars)
        if self._host_is_zh_media(final_url or url):
            traf_min = int(self.settings.zh_trafilatura_min_chars)

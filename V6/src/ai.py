...
class SummaryService:
    ...
    async def summarize_cluster(
        ...
        *,
        ...
        if len(items) == 1:
            one = items[0]
            raw_title = str(one.get("title", "")).strip()
            ...
            title_out = raw_title
            if self._text_is_mostly_non_cjk(raw_title):
                tzh = await self.translate_title_to_zh(title=raw_title)
                if tzh:
                    title_out = tzh
    ...
    async def translate_title_summary_to_zh(self, *, title: str, summary: str) -> tuple[str, str]:
        ...
        user_prompt = (
            ...
            "禁止出现「报道口径不一致」等元叙述。\n\n"
            f"{rules}\n\n"
            "标题须 12～28 字、一句话说清谁做了什么，不要整段摘要当标题。\n"
            "仅输出 JSON：{\"title_zh\":\"...\",\"summary_zh\":\"...\"}\n\n"
            f"标题：\n{(title or '')[:220]}\n\n摘要：\n{(summary or '')[:8000]}"
        )
        est = max(2400, min(8192, len(summary or "") // 2 + 1800))
        data = await self._chat_json(
        ...
        if not szh and summary and self._text_is_mostly_non_cjk(summary):
            szh = await self.translate_press_body_to_zh(title=title, body=summary)
        return tzh, szh

    async def translate_press_body_to_zh(self, *, title: str, body: str) -> str:
        """长通稿正文单独译为简体中文（标题+摘要联合调用失败时的兜底）。"""
        if not self._api_key:
            return ""
        raw = (body or "").strip()
        if not raw or not self._text_is_mostly_non_cjk(raw):
            return ""
        rules = self._load_editorial_rules()
        user_prompt = (
            "将下列公众号通稿正文改写为自然、流畅的简体中文，面向非专业读者，保持事实准确、语气中立。\n"
            "保留原文中的 <strong>...</strong> 重点句标签（整句包裹）；勿新增材料未写的事实与日期。\n"
            "禁止出现「报道口径不一致」等元叙述。\n\n"
            f"{rules}\n\n"
            '仅输出 JSON：{"body_zh":"..."}\n\n'
        ...
        est = max(2400, min(8192, len(raw) // 2 + 1800))
        data = await self._chat_json(
            system_prompt="你是专业翻译，只输出合法 JSON。",
            user_prompt=user_prompt,
            timeout=150.0,
        ...
        if not isinstance(data, dict):
            return ""
        return str(data.get("body_zh", "") or "").strip()

    @staticmethod
    def _text_is_mostly_non_cjk(text: str, *, max_cjk_ratio: float = 0.15) -> bool:
    ...
    async def translate_title_to_zh(self, *, title: str) -> str:
        """外文标题译为简体中文（用于尚未生成簇摘要的事件展示）。"""
        if not self._api_key:
            return ""
        raw = (title or "").strip()
        if not raw or not self._text_is_mostly_non_cjk(raw):
        ...
        if not isinstance(data, dict):
            return ""
        return str(data.get("title_zh", "") or "").strip()

    async def generate_compact_headline_zh(
        self,
        *,
        context_title: str = "",
        press_lede: str = "",
        ...
        """从事件语境/通稿首段生成 12–28 字中文标题（列表与推送用）。"""
        if not self._api_key:
            return ""
        from src.utils.headline_zh import trim_zh_headline_heuristic

        lede = (press_lede or "").strip()[:400]
        ctx = (context_title or "").strip()[:200]
        if not lede and not ctx:
            return ""
        data = await self._chat_json(
            system_prompt="你是科技媒体标题编辑，只输出合法 JSON。",
            user_prompt=(
                f"根据下列材料写一条中文新闻标题，{max(12, max_chars - 8)}～{max_chars} 字，"
                "一句话说清「谁+做了什么」，不要副标题、不要逗号串联两个长分句、"
                "不要「近日」「标志着」等套话。勿编造材料未写的事实。\n"
                f'仅输出 JSON：{{"headline_zh":"..."}}\n\n'
                f"英文原标题：{ctx}\n\n通稿首段：\n{lede}"
            ),
            timeout=60.0,
            max_tokens=120,
        ...
        if not isinstance(data, dict):
            return ""
        raw = str(data.get("headline_zh", "") or "").strip()
        return trim_zh_headline_heuristic(raw, max_len=max_chars) if raw else ""

    async def assess_domain_relevance(self, *, title: str, text: str, snippet: str = "") -> dict:
        """
        判断稿件是否属于低空经济 / 无人机政策监管 / 前沿科技产业新闻。

        返回: {"is_relevant": bool, "category": str, "reason": str}
        """
        fallback = self._fallback_domain_relevance(title=title, text=text, snippet=snippet)
        if not self._api_key:
            return fallback
        user_prompt = (
            "你是新闻领域编辑。判断下列稿件是否属于「低空经济、无人机/UAM/eVTOL、"
            "相关政策监管、产业与前沿科技」主线（含中文电报/快讯体例）。\n"
            "以下情况判为不相关：纯体育/娱乐/社会八卦/战争冲突/与低空产业无关的泛科技。\n"
            "仅输出 JSON：is_relevant(boolean), category(string: policy|frontier|industry|other), "
            "reason(string，20字内)。\n\n"
            f"标题：{title}\n\n摘要/导语：{(snippet or '')[:800]}\n\n正文：{(text or '')[:2000]}"
        )
        data = await self._chat_json(
            system_prompt="你是新闻领域门禁分类器，只输出 JSON。",
            user_prompt=user_prompt,
            timeout=45.0,
            max_tokens=200,
        )
        ...
        try:
            ...
            return {"is_relevant": is_relevant, "category": category, "reason": reason or fallback.get("reason", "")}
        except Exception:
            return fallback
    ...
    @staticmethod
    def _fallback_domain_relevance(*, title: str, text: str, snippet: str = "") -> dict:
        from src.utils.topic_prefilter import latin_low_altitude_keyword_hit
        ...
        if any(
            ...
            return {"is_relevant": True, "category": "industry", "reason": "keyword_hit"}
        if latin_low_altitude_keyword_hit(title=title, snippet=snippet, text=text):
            return {"is_relevant": True, "category": "industry", "reason": "latin_keyword_hit"}
        return {"is_relevant": False, "category": "other", "reason": "no_domain_signal"}

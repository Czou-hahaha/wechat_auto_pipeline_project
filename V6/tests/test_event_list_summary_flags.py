...
def test_needs_expansion_requires_seed() -> None:
    from src.bff.event_mapper import _needs_expansion_tag

    ev = {"id": "e0", "title": "No seed"}
    arts = [{"id": "a1", "status": "pending_summary", "title": "cluster only"}]
    assert _needs_expansion_tag(ev, arts, pushable=False) is False


def test_event_visible_in_list_requires_seed() -> None:
    from src.bff.event_mapper import event_visible_in_list

    ev = {"id": "e7", "title": "Expansion only"}
    arts = [{"id": "a1", "status": "event_enhancement", "title": "exp"}]
    assert event_visible_in_list(ev, arts) is False

    ev2 = {"id": "e8", "title": "Has seed"}
    arts2 = [{"id": "a2", "status": "ready_for_review", "title": "seed"}]
    assert event_visible_in_list(ev2, arts2) is True


def test_event_visible_in_list_map_primary_without_article_rows() -> None:
    from src.bff.event_mapper import event_visible_in_list

    ev = {"id": "e9", "title": "Map primary only"}
    roles = {"https://example.com/a": "primary", "https://example.com/b": "support"}
    assert event_visible_in_list(ev, [], url_roles=roles) is True

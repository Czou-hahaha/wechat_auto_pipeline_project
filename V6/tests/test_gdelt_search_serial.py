...
async def _run_gdelt_english_serial_test(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        Settings,
        "parsed_gdelt_english_keywords",
        lambda _self: ["drone", "eVTOL"],
    )
    monkeypatch.setattr(
        Settings,
        "parsed_chinese_keywords",
        lambda _self: ["低空经济"],
    )
    calls: list[str] = []
    ...
    with patch(
        "src.gdelt_search.gdelt_doc_search_with_client",
        new=AsyncMock(side_effect=fake_gdelt),
    ):

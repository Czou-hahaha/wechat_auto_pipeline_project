...
def test_extract_36kr_item_time_from_article_html() -> None:
    html = (
        '<article><h1>低空经济融资</h1>'
        '<span class="item-time">2026.06.07 18:30</span>'
        "<p>正文</p></article>"
    )
    soup = BeautifulSoup(html, "html.parser")
    settings = MagicMock()
    ...
    runner._published_css_by_host = {}
    runner.settings = settings
    iso, coarse = runner._extract_published_at(
        soup, page_url="https://36kr.com/p/1234567"
    )
    assert iso.startswith("2026-06-07")
    assert coarse is False

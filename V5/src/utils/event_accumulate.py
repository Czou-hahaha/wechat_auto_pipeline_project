...
def _span_hours(cluster: list[Any], existing_rows: list[dict[str, Any]]) -> float:
    times: list[datetime] = []
    for p in cluster:
...
def existing_event_by_urls(
    ...
    """任一新稿 URL 已在库中且带 event_id → 直接归入该 event。"""
    from src.storage import JsonStore
...
async def match_existing_event_id(
    *,
    ...
    title_min_overlap = 0.38

    candidates: list[tuple[str, list[dict[str, Any]], str, str]] = []
    for ev in events:
    ...
    if _embedding_available(settings):
        ...
        v0 = embeddings[0]
        if v0 is not None:
            for i, (eid, rows, _txt, ev_titles) in enumerate(candidates):
                vi = embeddings[i + 1]
                if vi is None:
                    continue
                ...
                if sim < t_link and overlap < title_min_overlap:
                    continue
                if _span_hours(cluster, rows) > span_limit:
                    continue
    ...
    if not best_id:
        for eid, rows, _txt, ev_titles in candidates:
            overlap = _title_overlap(cluster_title, ev_titles)

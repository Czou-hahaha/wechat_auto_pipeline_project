...
def _passes(score: int, hallucination: bool, approved: bool, *, threshold: int) -> bool:
    return (not hallucination) and approved and score >= threshold
...
async def run_press_quality_pipeline(
    *,
    ...
    threshold = int(
        ...
        else cfg.effective_qa_rewrite_pass_threshold()
    )
    max_rw = int(
        max_rewrite_rounds
        if max_rewrite_rounds is not None
        else cfg.effective_qa_rewrite_max_rounds()
    )
    ...
    current = draft.strip()
    prompt_contract = PromptContractStore(Path(cfg.data_dir)).get_editable()
    contract_hard_rules = [str(x).strip() for x in (prompt_contract.get("hard_rules") or []) if str(x).strip()]
    contract_soft_rules = [str(x).strip() for x in (prompt_contract.get("soft_rules") or []) if str(x).strip()]
    rounds: list[QualityRoundTrace] = []
    rewrite_attempts = 0
    stopped = "initial"

    for cycle in range(0, max_rw + 1):
        qa_res = await qa_svc.review(
            event_articles=event_articles,
            draft=current,
            max_event_articles_chars=max_ctx,
            contract_hard_rules=contract_hard_rules,
            contract_soft_rules=contract_soft_rules,
        )
        ok = _passes(
            qa_res.score,
            qa_res.hallucination,
            qa_res.approved,
            threshold=threshold,
        )

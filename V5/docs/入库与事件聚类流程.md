...
## 中文升级分支（V5 · `ZH_INGEST_LLM_ENABLED`）

中文源（`data_sources.json` 中 `language=zh`）在开关开启时走独立子路径，详见 [中文入库 LLM 流程](./中文入库LLM流程.md)：

1. **跳过** pre-fetch `topic_prefilter` 关键词门禁（仍保留时间窗、URL 去重、科技早报拼盘拦截）
2. 抓正文后 **跳过** `ZH_INGEST_MIN_CHARS` 字数门槛
3. **LLM 单篇门禁**（`src/services/zh_ingest/article_gate.py`）替代 `assess_domain_relevance` + `should_keep_editorial_content`
4. bge-m3 批内聚类不变
5. **LLM 簇级事件提炼**（`event_extract.py`）→ `events.json` 的 `title` / `title_zh`（约 20 字）
...
- **`src/utils/event_accumulate.py`**：`_seed_cluster` 落库前，用 **URL 命中** 或 **代表稿向量余弦 ≥ `EMBEDDING_EVENT_LINK_MIN`**（同窗 `CLUSTER_MERGE_HOURS`）匹配库内已有 event。
- 命中则 **只追加新 URL 的稿件** 与 `event_article_map`（角色 `support`），**不**再 `append_event`；避免同一合作新闻（如 ACSL–Draganfly）因不同 `topic_key` 或分次 `run_once` 拆成两个 event。

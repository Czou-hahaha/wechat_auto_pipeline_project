...
import { SourceIntakeWizard } from "@/components/settings/source-intake-wizard";
...
export default function SettingsPage() {
  ...
  function editBySourceId(sourceId: string) {
  ...
  return (
    <motion.div className="mx-auto max-w-5xl space-y-6">
      ...
      {loading ? (
        ...
        <>
          <SourceIntakeWizard onSaved={() => void load()} />
          <SourcesTable
          sources={sources}
          healthMap={healthMap}
          ...
          onAdd={openAdd}
          onEdit={openEdit}
          onEditBySourceId={editBySourceId}
...
function SourcesTable({
  ...
  onEditBySourceId?: (sourceId: string) => void;
            ...
            {healthAlerts.map((a) => (
              <li key={a.key} className="flex flex-wrap items-center justify-between gap-2 text-xs">
                ...
                <span className="flex gap-2">
                  {a.source_id && onEditBySourceId ? (
                    <button
                      type="button"
                      onClick={() => onEditBySourceId(String(a.source_id))}
            ...
            {sources.map((row, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-xs text-zinc-700">
                  {(() => {
                    const b = ingestModeBadge(row.ingest_mode);
                    return (
                      <Badge variant="muted" className={b.className}>
                        {b.label}
                      </Badge>
                    );
                  })()}

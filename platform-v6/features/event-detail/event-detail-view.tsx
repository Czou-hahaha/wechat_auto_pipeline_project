...
import { EventIntelligenceGraphPanel } from "@/components/events/event-intelligence-graph-panel";
...
export function EventDetailView({ id }: { id: string }) {
  ...
  const [detailTab, setDetailTab] = useState<"press" | "graph">("press");
  const [graphData, setGraphData] = useState<EventIntelligenceGraphPayload | null>(null);
  ...
  useEffect(() => {
    if (!id || detailTab !== "graph") return;
    let mounted = true;
    const loadGraph = async () => {
      setLoadingGraph(true);
      try {
        const res = await fetch(`/api/events/${id}/graph`);
        const body = await res.json().catch(() => null);
        if (!mounted) return;
        setGraphData(res.ok ? (body as EventIntelligenceGraphPayload) : null);
      } finally {
    ...
    try {
      ...
      setActionMsg({
        ...
        text: msg.includes("404") || msg.includes("Not Found")
          ? "标记失败：BFF 未更新，请重启 V6 BFF（:8788）后重试"
          : msg,
      });
    } finally {
      setPushing(false);
  ...
  async function refreshMemory() {
    ...
    try {
      ...
      if (!res.ok) throw new Error("记忆刷新失败");
      const graphRes = await fetch(`/api/events/${id}/graph`);
      const body = await graphRes.json().catch(() => null);
      if (graphRes.ok) setGraphData(body as EventIntelligenceGraphPayload);
    ...
    try {
      const res = await fetch("/api/feedback", {
        ...
        body: JSON.stringify({
          ...
          note: feedbackAnchor.trim()
            ? `[${feedbackAnchor.trim()}] ${feedbackNote.trim()}`
            : feedbackNote.trim(),
        }),
      });
      ...
      setFeedbackNote("");
      setFeedbackAnchor("");
      setFeedbackMsg({ ok: true, text: "反馈已提交，可在「反馈与策略」页查看汇总" });
    } catch (e) {
      setFeedbackMsg({
        ok: false,
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <div className="flex gap-2 border-b border-zinc-200 pb-1">
        <button
          type="button"
          onClick={() => setDetailTab("press")}
          className={cn(
            "rounded-t-lg px-4 py-2 text-sm",
            detailTab === "press"
              ? "border border-b-0 border-zinc-200 bg-white font-medium text-zinc-900"
        ...
        <button
          ...
          className={cn(
            "inline-flex items-center gap-1.5 rounded-t-lg px-4 py-2 text-sm",
            detailTab === "graph"
              ? "border border-b-0 border-zinc-200 bg-white font-medium text-zinc-900"
              : "text-zinc-600 hover:text-zinc-800",
          )}
      ...
      <CardStatic className="p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-zinc-700">反馈回流</h3>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            <option value="missing_context">缺国内参照</option>
          </select>
          <input
            value={feedbackAnchor}
            onChange={(e) => setFeedbackAnchor(e.target.value)}
            className="input-field w-36 text-xs"
            placeholder="锚点（段落/URL）"
          />
          <input
            value={feedbackNote}

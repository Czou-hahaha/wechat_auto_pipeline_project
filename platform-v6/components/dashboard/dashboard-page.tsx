...
export function DashboardPage() {
  ...
  useEffect(() => {
    fetch("/api/notifications")
      .then((r) => r.json())
      .then((body) => setNotifications(body as NotificationsPayload))
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-6 text-sm text-red-800">
          ...
          <p className="mt-1 text-red-700">
            {error instanceof Error ? error.message : "请确认 BFF 已在 :8788 运行"}
          </p>
        </div>
      </div>
    );
  ...
  const phaseEntries = Object.entries(data.phaseTimings?.phases || {}).map(
    ([name, sec]) => ({ name, sec: Number(sec) || 0 }),
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {(notifications?.total || data.notificationCount || 0) > 0 ? (
        <div className="flex flex-wrap items-start justify-between gap-3 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          <div className="flex items-start gap-2">
            <Bell className="mt-0.5 h-4 w-4 shrink-0" />
            <div>
              <p className="font-medium">
                {notifications?.total ?? data.notificationCount} 条数据源健康告警
              </p>
      ...
      {phaseEntries.length > 0 ? (
        <CardStatic>
          <h2 className="mb-2 text-sm font-medium text-zinc-700">
            最近 run-once 阶段耗时（{data.phaseTimings?.total_sec ?? 0}s）
          </h2>
          <div className="h-44">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={phaseEntries} margin={{ left: 8, right: 8, top: 4, bottom: 4 }}>
                <XAxis dataKey="name" stroke="#a1a1aa" fontSize={10} />
                <YAxis stroke="#a1a1aa" fontSize={10} unit="s" />
                <Tooltip formatter={(v: number) => [`${v}s`, "耗时"]} />
                <Bar dataKey="sec" fill="#6366f1" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </CardStatic>

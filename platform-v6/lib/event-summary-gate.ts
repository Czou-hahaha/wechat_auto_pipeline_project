...
function effectiveArticleCount(item: ListLike): number {
  const rows = Number(item.article_count ?? 0);
  const seed = Number(item.seed_article_count ?? 0);
  const exp = Number(item.expansion_article_count ?? 0);
  return Math.max(rows, seed + exp, seed);
}
...
export function resolveHasSummary(item: ListLike): boolean {
  if (item.hasSummary === true && (item.summaryKind ?? "") === "press") {
    return true;
  }
  const articleCount = effectiveArticleCount(item);
...
function shortNoSummaryReason(
  ...
  const n = effectiveArticleCount(item);
